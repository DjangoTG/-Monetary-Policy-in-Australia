...
rmarkdown::paged_table(results.adf2)

```

# Methodology

Below is the Structural Vector Autoregression (SVAR) model required to capture the dynamics between the variables.
...
Using this model will give me an insight into dynamic relationships between government spending, revenue and income inequality. Impulse response functions (IRFs) and forecast error variance decompositions (FEVDs) will be used to analyse the effects of fiscal policy on income inequality.
...
```{r}
hh            = 1:(h+1)
fevd.unemQ  = apply(FEVD.posterior[5,,,],1:2,mean)
fevd.unemQ  = rbind(rep(0,h+1),apply(fevd.unemQ,2,cumsum))
...
plot(hh,fevd.unemQ[1,], type="n", ylim=c(0,100), axes=FALSE, xlab="", ylab="")
axis(1,c(1,2,5,7,9,13,17),c("","1 quarter","1 year","6 quarters", "2 years", "3 years", "4 years"))
axis(2,c(0,50,100),c("","FEVD[unem.Q]",""))
for (n in 1:N){
  polygon(c(hh,(h+1):1), c(fevd.unemQ[n,hh],fevd.unemQ[n+1,(h+1):1]), col=colors[n],border=colors[n])
}
axis(4, (0.5*(fevd.unemQ[(1:N),(h+1)]+fevd.unemQ[2:(N+1),(h+1)]))[c(4,5)], c("au.mps","unem.Q"))
