...
p = 2
Y=C
sumofcoef.prior <- function(Y, X, p){
  in.obvs.Y <- Y[1:p,]
  ...
  for (i in 1:M){
    ...
    for (j in 1:N){
      dum.j = mean(in.obvs.Y[,j])
      if(i == j & j<M){
        X.D[j,i] = (dum.j/gamma.3)}
      else{X.D[j,i] = 0}
          }
  }
  ...
  for (i in 1:M){
    if(i <= N){
    dum.i = mean(in.obvs.Y[,i])
    X.temp[i] = dum.i/gamma.4}
  else{X.temp[i] = 1/gamma.4}}
...
dum = prior.ext.dummyset(X = L, Y = C)

X.2 <- dum$X.ex
Y.2 <- dum$Y.ex

A.hat = solve(t(dum$X.ex)%*%dum$X.ex)%*%(t(dum$X.ex)%*%dum$Y.ex)
Sigma.hat = diag(2)
